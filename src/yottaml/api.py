...
from yottaml.lib.utils import clean_none_value
from yottaml.lib.utils import encoded_string
...
class API(object):
    ...
    def http_patch(self, url_path, payload=None, headers=None):
        if payload is None:
            payload = {}
        ...
        if self.debug:
            self._logger.debug("http_patch raw response from server:" + response.text)

        self._handle_exception(response)
        return response.json()

    def http_put(self, url_path, payload=None, headers=None):
        if payload is None:
            payload = {}
        ...
        if self.debug:
            self._logger.debug("http_put raw response from server:" + response.text)
