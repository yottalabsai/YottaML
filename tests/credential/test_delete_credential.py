...
def test_delete_credential_success(credential_api):
    ...
    with patch.object(credential_api, "http_delete", return_value=MOCK_SUCCESS_RESPONSE) as mock_delete:
        ...
        mock_delete.assert_called_once_with(f"/v2/container-registry-auths/{credential_id}", payload=None)
...
def test_delete_credential_client_error(credential_api):
    ...
    with patch.object(
        credential_api,
        "http_delete",
        side_effect=ClientError(200, 25002, "Credential is in use and cannot be deleted.", {}, None),
    ):
