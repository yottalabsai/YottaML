...
def test_get_pods_empty_list(pod_api):
    with patch.object(
        pod_api, "http_get", return_value=MOCK_EMPTY_RESPONSE
    ) as mock_get:
...
def test_get_pods_single_pod(pod_api):
    with patch.object(
        pod_api, "http_get", return_value=MOCK_SINGLE_POD_RESPONSE
    ) as mock_get:
        response = pod_api.get_pods()
...
def test_get_pods_multiple_pods(pod_api):
    with patch.object(
        pod_api, "http_get", return_value=MOCK_MULTIPLE_PODS_RESPONSE
    ) as mock_get:
        response = pod_api.get_pods()
...
def test_get_pods_client_error(pod_api):
    with patch.object(
        pod_api,
        "http_get",
        side_effect=ClientError(400, 40001, "Invalid API key", {}, None),
    ):
